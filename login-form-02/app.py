...
def reg():
    ...
    conn=mysql.connector.connect(user="root",host="localhost",password="Anjana123#$",database="pythonlogin")
    cursor=conn.cursor()
...
def predict():
    ...
    print(movie)
    try:
     l=model.recommend(movie)
     print(l)
     return render_template("pred.html",users=l)
    except:
     return render_template("pred.html",err="Check the spelling or try another book")
